# IIS2DLPC Tap and Double Tap Detection on STM32F401RE Nucleo-64 Using Embassy Framework

This example demonstrates how to configure the **IIS2DLPC** ultra-low-power accelerometer for **single and double tap detection** on an **STM32F401RE Nucleo-64** board. The sensor is set up to detect tap events on all three axes, and the results (including tap sign and axis) are reported over UART.
...
- Tap duration, quiet, and shock parameters are configured for reliable detection.
- Both single and double tap detection modes are enabled.
- Tap interrupt is routed to INT1 (though this example uses polling).

### Event Polling Loop

- The main loop continuously polls the sensor’s status registers.
- If a double tap event is detected, a message is sent over UART indicating the sign and axis of the tap.
...
4. Tap or double tap the sensor on any axis to trigger events.
5. Observe messages like "Tap Detected: Sign positive on X axis" or "Double Tap Detected: Sign negative on Z axis" printed over UART.
...
*This README provides a detailed explanation of the embedded Rust program for single and double tap detection using the IIS2DLPC sensor on STM32F401RE with UART output, leveraging the Embassy async framework.*
