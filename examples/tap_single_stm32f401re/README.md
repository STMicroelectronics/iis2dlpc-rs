# IIS2DLPC Single Tap Detection on STM32F401RE Nucleo-64

This example demonstrates how to configure and use the **IIS2DLPC** accelerometer's embedded single tap detection feature on an **STM32F401RE Nucleo-64** board. The program sets up the sensor to detect single tap events on all axes and reports them over UART.
...
- Sets tap threshold to 9 on all axes.
- Configures single tap parameters: quiet and shock.
- Enables only single tap detection.
- Routes single tap detection interrupt to INT1 pin.

### Event Handling Loop

- Continuously polls the sensor for single tap events.
...
4. Tap the sensor; observe detection messages printed for each event and axis.
...
*This README explains the embedded Rust program for single tap detection using the IIS2DLPC sensor on STM32F401RE.*
