# IIS2DLPC Wake-Up Detection on STM32F401RE Nucleo-64

This example demonstrates how to configure and use the **IIS2DLPC** accelerometer's embedded wake-up detection feature on an **STM32F401RE Nucleo-64** board. The program sets up the sensor to detect motion events (wake-up) on any axis and reports them over UART.
...
- Sets Output Data Rate (ODR) to 200 Hz.
- Applies high-pass digital filter on the wake-up function.
- Sets wake-up duration to 0 (interrupt generated for each event exceeding threshold).
- Sets wake-up threshold to 2 (1 LSB = FS_XL/2^6).
- Enables wake-up interrupt generation on INT1 pin.

### Event Handling Loop

- Continuously polls the sensor for wake-up events.
- On **wake-up** detection:
  - Reports the event and the axis (X, Y, or Z) where the motion was detected over UART.
...
4. Move or shake the sensor; observe wake-up event messages printed for each axis.

---

## Notes

- The wake-up threshold and duration can be tuned for sensitivity and responsiveness.
...
*This README explains the embedded Rust program for wake-up detection using the IIS2DLPC sensor on STM32F401RE.*
